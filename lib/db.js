...
//look for activity of 1 or more, look for identical
//existing names, combine activity and delete the one with
//less activity
//TODO: stream this, more efficient method, case insensitive
Database.prototype.reduceAnalytics = function (cb) {
  Database.db.collection('analytics', function (err, users) {
    if (err) {
      cb(err)
    } else {
      users.find().toArray(function (err, hosts) {
        var done = hosts.length-1
        ...
        if (err) {
          ...
          hosts.forEach(function (host) {
            if (host.activity >= 1) {
              Database.db.collection('analytics').findAndModify(
                {nick: host.nick}, // query
                [['_id','asc']],  // sort order
                {$inc: {activity: host.activity}}, // replacement
                {}, // options
                function(err, object) {
                  if (err) {
                    cb(err)
                  } else {
                    users.remove(host, {w:1}, function (err) {
                      if (err) {
                        cb(err)
                      }
                    })
